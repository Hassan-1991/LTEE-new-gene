...
#This needs to be buttressed with blast analysis:
...
grep -v "#" "$i"_50000gen_[0-9][0-9][0-9][0-9][0-9].gd | sed "s/IS1\t-1/IS1_-1/g" | sed "s/IS1\t1/IS1_1/g" | sed "s/IS3\t-1/IS3_-1/g" | sed "s/IS3\t1/IS3_1/g" | sed "s/IS4\t-1/IS4_-1/g" | sed "s/IS4\t1/IS4_1/g" | sed "s/IS150\t-1/IS150_-1/g" | sed "s/IS150\t1/IS150_1/g" | sed "s/IS186\t-1/IS186_-1/g" | sed "s/IS186\t1/IS186_1/g" | egrep "DEL|CON|SUB|AMP" | awk -v var="$i" -F '\t' '{OFS=""}{print "REL606\t.\tCDS\t",$5-500,"\t",$5,"\t.\t-\t0\ttranscript_id \"",var,"_",$2,"_",$1,"_minus_up_500\";gene_id \"",var,"_",$2,"_",$1,"_minus_up_500\";"}' >> "$i"_ancestor.gtf
sort -t "_" -nk 3 "$i"_ancestor.gtf -o "$i"_ancestor.gtf
grep -v "#" "$i"_50000gen-applied.gd | grep -v "INV" | sed "s/IS1\t-1/IS1_-1/g" | sed "s/IS1\t1/IS1_1/g" | sed "s/IS3\t-1/IS3_-1/g" | sed "s/IS3\t1/IS3_1/g" | sed "s/IS4\t-1/IS4_-1/g" | sed "s/IS4\t1/IS4_1/g" | sed "s/IS150\t-1/IS150_-1/g" | sed "s/IS150\t1/IS150_1/g" | sed "s/IS186\t-1/IS186_-1/g" | sed "s/IS186\t1/IS186_1/g" | awk '{ for (i=1; i<=NF; i++) { if (i == 1 || i == 2 || i == 5 || i == 6 || $i ~ /(applied_start=|applied_end=)/) printf "%s ", $i } printf "\n" }' | sed "s/applied_end=//g" | sed "s/applied_start=//g" | awk -v var="$i" -F ' ' '{OFS=""}{print "REL606\t.\tCDS\t",$5,"\t",$5+500,"\t.\t+\t0\ttranscript_id \"",var,"_",$2,"_",$1,"_plus_down_500\";gene_id \"",var,"_",$2,"_",$1,"_plus_down_500\";"}' > "$i"_evolved.gtf
grep -v "#" "$i"_50000gen-applied.gd | grep -v "INV" | sed "s/IS1\t-1/IS1_-1/g" | sed "s/IS1\t1/IS1_1/g" | sed "s/IS3\t-1/IS3_-1/g" | sed "s/IS3\t1/IS3_1/g" | sed "s/IS4\t-1/IS4_-1/g" | sed "s/IS4\t1/IS4_1/g" | sed "s/IS150\t-1/IS150_-1/g" | sed "s/IS150\t1/IS150_1/g" | sed "s/IS186\t-1/IS186_-1/g" | sed "s/IS186\t1/IS186_1/g" | awk '{ for (i=1; i<=NF; i++) { if (i == 1 || i == 2 || i == 5 || i == 6 || $i ~ /(applied_start=|applied_end=)/) printf "%s ", $i } printf "\n" }' | sed "s/applied_end=//g" | sed "s/applied_start=//g" | awk -v var="$i" -F ' ' '{OFS=""}{print "REL606\t.\tCDS\t",$6-500,"\t",$6,"\t.\t-\t0\ttranscript_id \"",var,"_",$2,"_",$1,"_minus_up_500\";gene_id \"",var,"_",$2,"_",$1,"_minus_up_500\";"}' >> "$i"_evolved.gtf
sort -t "_" -nk 3 "$i"_evolved.gtf -o "$i"_evolved.gtf
done
...
for i in Ara+1 Ara+2 Ara+3 Ara+4 Ara+5 Ara-1 Ara-2 Ara-3 Ara-4 Ara-5 Ara-6
do
gffread -E -w "$i"_ancestor.faa -g ../../../all_genomes/REL606.fasta "$i"_ancestor.gtf
/stor/work/Ochman/hassan/tools/gmap-2021-05-27/bin/gmap -D .. -d "$i" -f 2 --gff3-fasta-annotation=1 "$i"_ancestor.faa > "$i"_ancestor.gff3 #map to evolved genomes
awk -F '\t' '($3=="mRNA")' "$i"_ancestor.gff3 | grep "mrna1" | cut -f 1-3 -d '.' | awk -F '\t' '{OFS=FS}{print $1,$2=".",$3="CDS",$4,$5,$6,$7,$8,"transcript_id \"",$9,"\";gene_id \"",$9,"\";"}' | sed "s/\tID=//g" | sed "s/\t\"/\"/g" > "$i"_evolved_gmapped.gtf
done
